...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        List<Emp> result = emps.Where(e => e.DeptNo.Equals(depts.Where(d => d.Loc.Equals("CHICAGO")).Select(d => d.DeptNo).FirstOrDefault())).ToList(); 
    ...
    public void ShouldCountEmployeesPerDepartment()
    {
        ...
        var result = emps.GroupBy(e => e.DeptNo).Select(group => new
        {
            DeptNo = group.Key,
            Count = group.Count()
        }).ToList(); 
    ...
    public void ShouldReturnEmployeesWithCommission()
    {
        ...
        var result = emps.Where(e => e.Comm != null).Select(e => new
        {
            e.EName, e.Comm
        }).ToList(); 
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        var result = emps.SelectMany(e => grades.Where(s => e.Sal >= s.Losal && e.Sal <= s.Hisal),(e, s) => new
                {
                    e.EName,
                    s.Grade
                })
