...
public class AdvancedEmpDeptTests
{
    ...
    public void ShouldReturnMinSalaryInDept30()
    {
        ...
        decimal? minSalary = emps.Where(e => e.DeptNo == 30).Select(emp => emp.Sal).Min();
    ...
    public void ShouldJoinEmpDeptSalgrade()
    {
        ...
        var result = emps
            .SelectMany(e => grades.Where(s => e.Sal >= s.Losal && e.Sal <= s.Hisal), (e, s) => new { e, s }).Join(depts, es => es.e.DeptNo, d => d.DeptNo, (es, d) => new
                {
                    es.e.EName,
                    es.s.Grade,
                    d.DName
